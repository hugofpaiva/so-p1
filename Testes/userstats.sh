...
# Arrays para guardar users e a sua informação
...
function getUserInfo() {
	...
	for user in ${users[@]}; do
		...
		# OPÇÃO -s -e
			...
			sessions=$(eval last $start $end| grep -o $user | wc -l)
			time=$(eval last $start $end | grep $user | awk '{print $10}' | sed '/in/d' | sed 's/[)(]//g')
